...
import (
	...
	driver "github.com/shanna/migrate/driver/duckdb"
)

func TestDuckDBMigrateCommit(t *testing.T) {
	...
	migrator, err := driver.New(dir + "/migrate.db")
	if err != nil {
	...
	db, err := sql.Open("duckdb", dir+"/migrate.db")
	if err != nil {
...
func TestDuckDBMigrateRollback(t *testing.T) {
	dir, err := os.MkdirTemp("", "migrate-*")
	if err != nil {
		panic(err)
	}
	...
	var migrations = []struct{ name, sql string }{
		{`migrate rollback: create table sql`, `create table rollback_test (id integer, name text)`},
...
func TestDuckDBMigrateChecksumRollback(t *testing.T) {
	dir, err := os.MkdirTemp("", "migrate-*")
	if err != nil {
		panic(err)
	}
	...
	var migrations = []struct{ name, sql string }{
		{`migrate checksum collision`, `create table rollback_test (id integer, name text)`},
